...
#print stock
minex_stock = min(stock)
maxex_stock = max(stock)

from sklearn.preprocessing import MinMaxScaler
Scaler = MinMaxScaler()

def sal(x): 
    salary_weight = numpy.array([[float(minSalary)],[float(x)],[float(maxSalary)]])
    rescaled_salary_weight = Scaler.fit_transform(salary_weight)
    return rescaled_salary_weight[1]
...
print finance_features[0][0]
z=finance_features[0][0]
print sal(z)


print range(len(finance_features))
for i in len(range(finance_features)):
#    z = finance_features[i][0]
